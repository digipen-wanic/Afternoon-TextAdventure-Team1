...
Room* Room1_Build()
{
	...
	/* Create the room
	include an initial room description */
	room1 = Room_Create("A ha this is other room in the jail level\n");
	/* Exits
	add one or more exits to allow navigation between rooms */
	Room_AddRoomExit(room1, "swap", 0);

	/* Items
	add items to the room */
	ItemList_AddItem(Room_GetItemList(room1), ExitDoor_Build());
